...
### Summary Checklist for Manual Review:
- [ ] Review proposed IDOR fix in `SetBranchContext.php`
- [ ] Add `ProtectedRoute.jsx` component and wrap protected routes in `App.jsx`
- [ ] Update `SANCTUM_STATEFUL_DOMAINS` in `.env` for future tenant subdomains
- [ ] Remove legacy `mockUser.token` reference in `DashboardLayout.jsx`
